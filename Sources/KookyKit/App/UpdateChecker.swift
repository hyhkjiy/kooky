...
/// against `KookyApp.displayVersion`. Manual-only for now (Help → Check
...
enum UpdateChecker {
    ...
    static func check(currentVersion: String = KookyApp.displayVersion) async -> Outcome {
        var request = URLRequest(url: KookyApp.releasesAPIURL)
